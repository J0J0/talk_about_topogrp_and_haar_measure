...
\item\label{tg:topogrpbsp:krulltopo}
...
\begin{proof}
    ...
    auch
    \[ \bigl( (a,x) \mapsto ax \bigr) \circ \bigl( x\mapsto (a,x) \bigr)
    \]
...
\begin{thKorollar}%
    ...
    Dann sind auch $a\mkern1mu U$ und $U\mkern-2mu a$ offen sowie $aV$ und $V\mkern-2mu a$
    abgeschlossen.  Außerdem sind auch $B\mkern1mu U$ und $U\mkern-2mu B$ offen.
\end{thKorollar}
...
\begin{proof}
    ...
    \[  B\mkern1mu U  = \bigcup_{b\in B} b\mkern1mu U 
        \qqundqq 
        U\mkern-2mu B = \bigcup_{b\in B} U\mkern-1mu b, \]
    wodurch wir erkennen, dass $B\mkern1mu U$ und $U\mkern-2mu B$ als
    Vereinigungen offener Mengen jeweils wieder offen sind. 
    \\
\end{proof}

Aus den letzten beiden Resultaten erhalten wir insbesondere, dass wir für $a\in
G$ und eine Umgebung $U\subset G$ von $e$ immer eine Umgebung $a\mkern1mu U$ um
$a$ mit gleichen topologischen Eigenschaften wie $U$ erhalten. Da Umgebungen um
$e$ auch weiterhin eine große Rolle spielen werden, definieren wir:
...
\begin{thKorollar}%
    ...
    %
    Wenn $G$ eine (lokalkompakte) topologische Gruppe ist, welche \emph{nicht}
    ...
    %
       \abs{ f(x) - f(xy) } = \abs{f(xy)}
        \quad &\leq \quad
          ...
          \quad + \quad
             \underbrace{ \abs{f(x_j)} }_{ \hspace*{40pt} \mathclap{
                \overset{\eqref{tg:unicont:p:haveineq}}{<} \frac{\epsilon}{2},
                \,\smash{
                \begin{aligned}[t]
                    &\text{{\scriptsize mit }$\scriptstyle y=x_j^{-1}x$} \\[-0.35\baselineskip]
                    &\text{\scriptsize in \eqref{tg:unicont:p:haveineq}}
                \end{aligned} }%
             } }
        \quad < \quad \epsilon
    \end{align*}
...
\begin{thDef}[Links-/Rechtsinvariantes Maß / Funktional]\hfill\\
...
Eindeutigkeitsbeweis des Haar-Maßes \pcref{chap:pf} benötigen werden.
...
\begin{thSatz}[Allgemeine Transformationsformel]
    \label{tg:trafo}
    %
    Seien $(X,\mathcal{A},\mu)$ ein Maßraum, $(Y,\mathcal{A}')$ ein
    messbarer Raum, $\phi\colon X\to Y$ eine
    $\mathcal{A}$"~$\mathcal{A}'$"~mess\-ba\-re
    Abbildung und $f\colon Y\to\C$ eine $\mathcal{A}'$"~mess\-ba\-re Funktion.
    Ist dann $\mu_\phi \defeq \mu \circ \phi^{-1}$ das von $\mu$ und $\phi$
    induzierte Bildmaß auf $(Y,\mathcal{A}')$, dann gilt:
    \\
...
zwischen invarianten Maßen und Funktionalen auf LKH-Gruppen:
\begin{thSatz}%
    [Invariante Radon-Maße entsprechen invarianten positiven Funktionalen]
    \label{tg:rmeasuresVSfunctionals}
    %
    Ist $I$ ein linksinvariantes positives Funktional auf $\contcomp(G)$, so
    existiert genau ein linksinvariantes Radon-Maß~$\mu$ so dass für alle
    $f\in\contcomp(G)$ gilt:
    \[ I(f) = \int_G f \dif\mu \]
...
\begin{proof}
    ...
    Formel erfüllt, liefert uns der Riesz'sche Darstellungssatz \pcref{tg:riesz}
...
\item\label{tg:haarbsp:matgroup}
    ...
    Wie wir jedoch später \pcref{sec:haaruniq}
...
\mycref{tg:topogrpbsp:krulltopo}) anfangen kann, findet man beispielsweise bei
